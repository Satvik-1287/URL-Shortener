...
export async function generateShortUrl(req, res) {
    try {
        ...
        res
            .status(500)
            .send({ message: "There is some error. Please come back later.", errString: error.message, });
    }
}
...
export async function redirectToUrl(req, res) {
    try{
        ...
        return res.status(500).send({ message: "There is some error. Please try again later.", errString: error.message, });
    }
}